    ...
    if (self.opts['react-docgen'].enabled) {
      ...
      uniqueFiles.forEach(function (file) {
        try {
          ...
          var reactDocGenId = propMetadata.props.reactDocGenId

          if (reactDocGenId && reactDocGenId.defaultValue && reactDocGenId.defaultValue.value) {
            // need to strip the quotes from the id, since the parser returns it like this:
            //  defaultValue: { value: '\'theComponentId\'', computed: false } }

            var id = reactDocGenId.defaultValue.value.replace(/'/g, '')

            if (self.reactPropMetas[id]) {
              reject(new Error('reactDocGenId already exists: ' + id))
            } else {
              self.reactPropMetas[id] = propMetadata
            }
          }
