...
function split(string) {
  return string.split(',').map(function(part) { return part.trim(); });
}
...
function RSG(opts) {
  ...
  opts.files = opts.files ? split(opts.files) : null;
  opts.babelConfig = readBabelRc() || opts.babelConfig || {};
  this.opts = opts;
}
