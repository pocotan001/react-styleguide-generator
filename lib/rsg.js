...
var DocgenPlugin = require('./webpack-plugins/react-docgen')
...
function RSG (input, opts) {
  ...
  this.stageDir = path.resolve(process.cwd(), 'rsg-tmp')
}
...
RSG.prototype.createContentsFile = function () {
  ...
  var webpackEntries = [contentsInter]
  ...
  function distContentsFile () {
    return new Promise(function (resolve, reject) {
      webpack(self.getWebpackConfig(webpackEntries), function (err, stats) {
        if (err) {
          return reject(err)
        }
  ...
  if (self.opts.dev) {
    hmrServer(self.getWebpackConfig(webpackEntries), self.opts.dev, self.log, path.join(self.stageDir, 'dist'))
    return true
  } else {
...
RSG.prototype.getWebpackConfig = function (componentEntries) {
  ...
  return {
    ...
    module: {
      loaders: [{
        ...
        include: [appDir].concat(self.input)
      },
      {
        test: /\.css$/,
...
RSG.prototype.dist = function () {
  var self = this

  return new Promise(function (resolve, reject) {
    var src = path.join(self.stageDir, 'dist')
    var dest = self.opts.output
