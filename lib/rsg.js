...
RSG.prototype.createReactFile = function() {
  var version = (pkg.dependencies || pkg.devDependencies || pkg.optionalDependencies)['react'];
  var suffix = /^git:|\*/.test(version) ? '' : '_v' + version.replace(/\W/g, '');
  var outFile = this.opts.output + '/src/_react' + suffix + '.js';

  if (suffix && fs.existsSync(outFile)) {
