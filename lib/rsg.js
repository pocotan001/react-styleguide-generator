...
function RSG (opts) {
  ...
  opts.output = opts.output ? opts.output.replace(/\/+$/, '') : 'styleguide';
  opts.title = opts.title || 'Style Guide';
  opts.basePath = path.normalize('/' + (opts.basePath ? opts.basePath.replace(/\/+$/, '') : opts.output));
...
RSG.prototype.generate = function(callback) {
  ...
  return Promise
    .all([
      this.copy(SRC.concat(this.opts.files)),
      html(this.opts),
