        ...
        if (!self.reactDocGenCache[file] || self.reactDocGenCache[file] && self.reactDocGenCache[file] !== fileStat.mtime) {
          ...
          if (displayName) {
            ...
            if (self.reactPropMetas[displayName] && !self.reactDocGenInit) {
              reject('duplicate displayName "' + displayName + '" in ' + file)
            } else {
...
RSG.prototype.createHtmlFile = function () {
  ...
  var data = assign({}, this.opts, {
    ...
    jsFils: this.jsFiles,
    livereload: this.opts.livereload
  })
...
RSG.prototype.createLivereloadFile = function () {
  var dest = this.opts.output + '/src/livereload.js'

  if (fs.existsSync(dest)) { return Promise.resolve() }

  return new Promise(function (resolve, reject) {
    fs.copy(path.join(__dirname, '..', 'node_modules', 'livereload', 'ext', 'livereload.js'), dest, function (err) {
      if (err) { return reject(err) }
      resolve()
    })
  })
...
RSG.prototype.createPropsDocFile = function () {
  var self = this

  return self.genReactPropDoc().then(function () {
    var dest = self.opts.output + '/src/propsdoc.js'
    var propMetas = JSON.stringify(self.reactPropMetas)
    var data = 'window.RSG = {' +
        ' propMetas: ' + propMetas +
      '}'

    return new Promise(function (resolve, reject) {
      fs.outputFile(dest, data, function (outputFileErr) {
        if (outputFileErr) {
          return reject(outputFileErr)
        }
        resolve()
      })
    })
...
RSG.prototype.createContentsFile = function () {
  ...
  if (self.opts.watch) {
    ...
    b.on('update', function () {
      ...
      Promise.all([
        self.createPropsDocFile(),
        distContentsFile()
      ]).then(function () {
        return self.createHtmlFile()
      }).catch(function (err) {
...
RSG.prototype.generate = function (cb) {
  ...
  return Promise
    .all([
      ...
      this.createLivereloadFile(),
      this.createContentsFile(),
      this.createPropsDocFile()
    ])
    .then(function () { cb() })
    .catch(function (err) {
      this.log.error(err)
