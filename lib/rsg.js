    ...
    if (self.opts['reactDocgen'].enabled) {
      ...
      uniqueFiles.forEach(function (file) {
        try {
          ...
          log.error(e, 'react-gendoc error')
          reject(e)
        }
      })
    }
  ...
  if (self.opts.watch) {
    ...
    b.on('update', function() {
        Promise.all([
          ...
          log.info('styleguide updated')
        }).catch(function(err) {
          log.error(err, 'browserify update error')
        })
    })

    b.on('error', function(err) {
      log.error(err, 'browserify error')
    })

    b.on('time', function(time) {
      log.info('styleguide rebundled in ' + time + 'ms')
    })
